...
class SlidingWindowDataLoader:
    ...
    def __len__(self) -> int:
        ...
        Returns the length of the data loader.
        """
        length = len(self.dataset) - self.history_length - self.prediction_length + 1
        print(f"[DEBUG] SlidingWindowDataLoader.__len__: Dataset length = {len(self.dataset)}, "
              f"History length = {self.history_length}, Prediction length = {self.prediction_length}, "
              f"Calculated length = {length}")
        return length
...
def forecasting_data_provider(data, config, timeenc, batch_size, shuffle, drop_last):
    ...
    print(f"[DEBUG] forecasting_data_provider: Dataset length = {len(dataset)}, "
          f"History length = {config.seq_len}, Prediction length = {config.pred_len}, "
          f"Label length = {config.label_len}")

    print(f"[DEBUG] DataLoader initialization: Dataset type = {type(dataset)}, Dataset length = {len(dataset)}")
    data_loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=shuffle,
    ...
    print(f"[DEBUG] forecasting_data_provider: DataLoader initialized with batch size = {batch_size}, "
          f"Shuffle = {shuffle}, Drop last = {drop_last}")
...
class DatasetForTransformer:
    ...
    def __len__(self) -> int:
        ...
        """
        length = len(self.dataset) - self.history_length - self.prediction_length + 1
        print(f"[DEBUG] DatasetForTransformer.__len__: Dataset length = {len(self.dataset)}, "
              f"History length = {self.history_length}, Prediction length = {self.prediction_length}, "
              f"Calculated length = {length}")
        return length
    ...
    def __read_data__(self):
        df_stamp = self.dataset.reset_index()
        print(f"[DEBUG] DatasetForTransformer.__read_data__: Time data before processing =\n{df_stamp.head()}")

        df_stamp = df_stamp[["date"]].values.transpose(1, 0)
        data_stamp = get_time_mark(df_stamp, self.timeenc, self.freq)[0]
        print(
            f"[DEBUG] DatasetForTransformer.__read_data__: Time encoding completed. Encoded shape = {data_stamp.shape}")
        self.data_stamp = data_stamp
...
class SegLoader(object):
    ...
    def __len__(self):
        ...
        """
        length = (self.data.shape[0] - self.win_size) // self.step + 1
        print(f"[DEBUG] SegLoader.__len__: Data shape = {self.data.shape}, Window size = {self.win_size}, "
              f"Step = {self.step}, Calculated length = {length}")
        if self.mode == "train":
